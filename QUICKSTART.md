...
### Easy Configuration
All bank rules are in one place: `BANKS_CONFIG` in `main.py`
...
Simply modify its `constraints` list in `BANKS_CONFIG`.
...
| File | Purpose |
|------|---------|
| `main.py` | FastAPI app & bank configuration |
| `models.py` | Pydantic data models |
| `constraints.py` | Constraint strategy implementations |
| `repository.py` | Bank data abstraction |
| `services.py` | Core matching logic |
...
Switching to a database requires only changing `repository.py`:
```python
...
5. Add more banks to `BANKS_CONFIG`
6. Add more constraint types as needed
7. Deploy to production when ready

## Documentation

See `README.md` for full architecture documentation and design patterns.
